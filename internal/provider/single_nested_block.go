...
func (g GeneratorSingleNestedBlock) CustomTypeAndValue(name string, generated map[string][]byte) ([]byte, error) {
	return schema.DedupeGenerated(name, generated, func() ([]byte, error) {
		...
		for _, k := range blockKeys {
			...
			// changed for conflict resolution.
			effectiveName := k
			if t, ok2 := g.Blocks[k].(schema.EffectiveTypeName); ok2 {
				if tn := t.EffectiveTypeName(); tn != "" {
					effectiveName = tn
				}
			}
...
func (g GeneratorSingleNestedBlock) EffectiveTypeName() string {

...
func (g GeneratorSingleNestedAttribute) CustomTypeAndValue(name string, generated map[string][]byte) ([]byte, error) {
	...
	renderChildren := func() ([]byte, error) {
		...
		for _, k := range attributeKeys {
			...
			effectiveName := k
			if t, ok2 := g.Attributes[k].(schema.EffectiveTypeName); ok2 {
...
func (g GeneratorSingleNestedAttribute) EffectiveTypeName() string {
	return g.CustomType.TypeName()
}

// WithResolvedTypeName returns a copy of the generator with the type name
// updated for conflict resolution.
func (g GeneratorSingleNestedAttribute) WithResolvedTypeName(name string) schema.GeneratorAttribute {
	g.CustomType = g.CustomType.WithTypeName(name)
	return g
}

...
func (g GeneratorListNestedAttribute) CustomTypeAndValue(name string, generated map[string][]byte) ([]byte, error) {
...
// EffectiveTypeName returns the type name that should be used for generating
// custom Type/Value types. This may differ from the attribute name if a
// type name conflict was resolved.
func (g GeneratorListNestedAttribute) EffectiveTypeName() string {
	return g.NestedAttributeObject.TypeName()
}

// WithResolvedTypeName returns a copy of the generator with the type name
// updated for conflict resolution.
func (g GeneratorListNestedAttribute) WithResolvedTypeName(name string) schema.GeneratorAttribute {
	g.NestedAttributeObject = g.NestedAttributeObject.WithTypeName(name)
	return g
}
